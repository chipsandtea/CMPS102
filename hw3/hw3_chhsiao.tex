...
\textbf{For $j \geq 4$:}\\
Our job is to consider our options between $A$ and $B$, and pick the smaller one. Option $A$ is to calculate the cost of the current round with $(r \cdot s_j)$, and add it to the running sum of minimum costs, represented by $OPT(j-1)$.
\end{proof}
